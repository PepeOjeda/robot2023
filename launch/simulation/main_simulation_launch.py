...
def launch_setup(context, *args, **kwargs):
    ...
    rviz =  Node(
        ...
        executable='rviz2',
        name='rviz2',
        arguments=['-d', os.path.join(my_dir, 'launch', 'robot2023.rviz')],
        output="log",
        prefix='xterm -hold -e',
        remappings=[
    ...
    robot1 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('robot2023'),'launch', 'single_robot_nav.py' )
        ),
        launch_arguments={
            'namespace': 'Robot1',
    ...
    tdlas_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('robot2023'),'launch', 'tdlas_test.py' )
        ),
        launch_arguments={
            'namespace': 'Robot1',
        }.items()
